-- Assign sold combos/products and post-sale extras to the production card chosen
-- during a split. The original sale item stays intact for commercial reporting.
BEGIN;
...
BEGIN
  ...
  FOR payment IN SELECT * FROM public.job_payments WHERE job_id = original.id ORDER BY id LOOP
    ...
    FOR target IN SELECT * FROM public.jobs
      ...
      share := CASE WHEN allocation_total = 0 THEN
        CASE WHEN target.id = first_key THEN payment.amount ELSE 0 END
      ELSE round(payment.amount * cumulative / allocation_total, 2) - allocated END;
