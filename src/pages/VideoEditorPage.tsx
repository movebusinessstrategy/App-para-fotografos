...
if (!isLocal) {
  fetch(`${RENDER_ORIGIN}/api/health`).catch(() => {});
}
...
export default function VideoEditorPage() {
  ...
  const handleRestoreJob = async (jobId: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/restore/${jobId}`);
      if (!res.ok) throw new Error('Job não encontrado.');
      await res.json();
      navigate(`/video-preview/${jobId}`);
    } catch (err: any) {
      setError(err.message || 'Não foi possível abrir este job.');
    } finally {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      formData.append('editOptions', JSON.stringify(editOpts));
      ...
      if (!uploadRes.ok) {
        throw new Error(uploadData.error || `Upload falhou (${uploadRes.status})`);
      }

      navigate(`/shorts/${uploadData.jobId}`);
    } catch (err: any) {
  ...
  const hasAnyAdvanced =
    editOpts.autoCut || editOpts.dynamicZoom || editOpts.broll || editOpts.sfx || editOpts.transitions;
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Upload → IA analisa → Preview Remotion → Render no AWS Lambda
          </p>
        </div>
      ...
      {step === 'uploading' ? (
        <div className="flex flex-col items-center justify-center py-16 space-y-4 rounded-2xl border border-gray-200 dark:border-gray-800">
          <Loader2 size={34} className="animate-spin text-indigo-500" />
          <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">Enviando vídeo...</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">Você será redirecionado para o preview em instantes.</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          >
            ...
            <p className="text-lg font-semibold text-gray-700 dark:text-gray-300">Arraste seu vídeo aqui ou clique para selecionar</p>
          ...
          {selectedFile && (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              <div className="lg:col-span-3 space-y-4">
                ...
                {activeTab === 'edits' && (
                  <div className="space-y-3">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      A IA analisa a transcrição do vídeo e aplica as edições antes do render final.
                    </p>
                    ...
                    {hasAnyAdvanced && (
                      <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
                        ...
                        <p className="text-xs text-amber-700 dark:text-amber-300">
                          O pipeline vai transcrever com Whisper, gerar plano de edição com GPT e montar o preview em Remotion.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                >
                  <Sparkles size={20} />
                  Ir para Preview Remotion
                </button>

                <p className="text-xs text-gray-400 text-center">
                  Pipeline: Upload → Whisper → GPT-4o → Remotion Preview → AWS Lambda Render.
                </p>
              </div>
            </div>
          )}
      ...
      {step === 'upload' && pastJobs.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            {loadingJobs && <Loader2 size={12} className="animate-spin text-gray-400" />}
          </div>
                ...
                <div className="w-20 h-28 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 group-hover:border-indigo-400 transition-colors">
                  ...
                  {job.hasEdited && (
                    <div className="absolute top-1 left-1">
                      <Sparkles size={12} className="text-green-400 drop-shadow" />
                    </div>
                  )}
