...
export default function PipelineSettings() {
  const [activeTab, setActiveTab] = useState<"vendas" | "producao">("vendas")
  ...
  async function fetchProductionStages() {
    try {
      const res = await authFetch('/api/production/stages')
      if (!res.ok) throw new Error('Erro ao carregar')
      const data = await res.json()
      setProductionStages(data)
    } catch (err) {
      console.error('Erro ao carregar estágios de produção:', err)
    }
  }

  const targetStages = activeTab === "vendas" ? stages : productionStages
  const refresh = activeTab === "vendas" ? fetchStages : fetchProductionStages
  const setTargetStages = activeTab === "vendas" ? setStages : setProductionStages
  ...
  async function handleAddStage() {
    ...
    const regularStages = targetStages.filter(s => !s.is_final)
    ...
    try {
      const res = await authFetch(activeTab === "vendas" ? '/api/pipeline/stages' : '/api/production/stages', {
        method: 'POST',
  ...
  async function handleUpdateStage(id: string) {
    ...
    try {
      const res = await authFetch(`${activeTab === "vendas" ? '/api/pipeline/stages' : '/api/production/stages'}/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleConfirmDelete() {
    ...
    try {
      const res = await authFetch(`${activeTab === "vendas" ? '/api/pipeline/stages' : '/api/production/stages'}/${stage.id}`, {
        method: 'DELETE'
      })
  ...
  async function handleDragEnd() {
    ...
    setSaving(true)
    try {
      const stageIds = targetStages.filter(s => !s.is_final).map(s => s.id)
      
      const res = await authFetch(activeTab === "vendas" ? '/api/pipeline/stages/reorder' : '/api/production/stages/reorder', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const currentStages = activeTab === "vendas" ? stages : productionStages
  const regularStages = currentStages.filter(s => !s.is_final)
  const finalStages = currentStages.filter(s => s.is_final)
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          Personalize os estágios dos seus pipelines
        </p>
