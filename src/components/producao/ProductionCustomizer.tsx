...
import { X, Plus, Trash2, GripVertical, Check, Pencil, Settings } from 'lucide-react';
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col border border-transparent dark:border-gray-800 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {/* ── Processos ── */}
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">Processos</p>
                    ...
                    {editingProcessId === p.id ? (
                      <div className="flex-1 flex items-center gap-1" onClick={e => e.stopPropagation()}>
                        <input
                          ...
                          className="flex-1 px-2 py-0.5 text-sm border border-indigo-400 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white outline-none"
                        />
                      ...
                      <>
                        <span className={`flex-1 text-sm font-medium ${activeProcessId === p.id ? 'text-indigo-700 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-200'}`}>
                          {p.name}
                        </span>
                        <button
              ...
              <div className="flex gap-2 mt-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:border-indigo-400"
                />
                <button
                  onClick={addProcess}
    ...
    >
      ...
      {isEditing ? (
        ...
        <>
          ...
          <button onClick={onStartEdit} className="opacity-0 group-hover:opacity-100 p-1 text-gray-400 hover:text-indigo-500 transition-all">
            <Pencil size={13} />
          </button>
