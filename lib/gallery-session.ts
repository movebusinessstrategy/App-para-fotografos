...
function getKey(): Buffer {
  ...
  // Fallback: chave estática derivada do APP_PUBLIC_URL. Não é segura,
  // mas evita travar dev. Logamos um aviso no boot — não em cada uso.
...
export function signGallerySession(
  ...
  const sig = crypto.createHmac('sha256', getKey()).update(payloadStr).digest();
...
export function verifyGallerySession(token: string | null | undefined): GallerySessionPayload | null {
  ...
  const expected = crypto.createHmac('sha256', getKey()).update(payloadStr).digest();
