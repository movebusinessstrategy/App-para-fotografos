# CRM Trilha - Extensão Chrome para WhatsApp Web
...
1. Clique no ícone da extensão CRM Trilha na barra do Chrome
2. Cole o token JWT do CRM Trilha (Supabase auth token)
3. Clique em **Salvar configurações** - a extensão vai testar o token automaticamente

### Como obter o token
No CRM Trilha, abra o DevTools (F12) → Application → Local Storage → `sb-*-auth-token`
Copie o valor de `access_token`.

> **Dica:** Em breve haverá um botão "Copiar token da API" direto no CRM Trilha.

## Como usar

1. Abra [web.whatsapp.com](https://web.whatsapp.com) no Chrome
2. O painel lateral do CRM Trilha aparece automaticamente no lado direito
...
O dashboard do CRM Trilha (Vendas) mostra o Kanban e as métricas - sem inbox.
