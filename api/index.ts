  ...
  try {
    ...
    if (pathParts[0] === 'deals') {
      ...
      if (req.method === 'GET') {
        ...
        const deals = (dealsRes.data || []).map(deal => ({ ...deal, client_name: deal.client_id ? clientMap.get(deal.client_id) || null : null }));
      ...
      if (req.method === 'POST') {
        ...
        const { data, error } = await supabase.from('deals').insert({
          client_id: client_id || null, title, value: value || 0, stage: stage || 'lead', priority: priority || 'medium',
          expected_close_date: expected_close_date || null, next_follow_up: next_follow_up || null, notes: notes || null,
